...
import {Accordion, AccordionContext, Card, OverlayTrigger, useAccordionButton} from "react-bootstrap"
...
function Grommet({CatID, ModelID, SpecialId, ProjectId, EditIndex, PageItem, QueryString, Parameters, PageId}) {
    ...
    const [depSet, setDepSet] = useState(new Set(['1', '2', '3', '4', '5', '6', '7', '8', '1001', '1002']));
    ...
    const filterCheckboxes = useRef({
        "colors": [], "patterns": [], "types": [], "prices": []
    });
    const steps = useRef([]);
    const draperyRef = useRef([]);
    
    const [filterChanged, setFilterChanged] = useState({
        filter: 0, filter_id: undefined, isDelete: false
    });
    ...
    function getFabricsWithFilter(clearAll) {
        if (clearAll) {
            ...
            Promise.all([promise1, promise2, promise3, promise4, promise5]).then(() => {
                ...
                axios.get(baseURLFabrics, {
                    params: paramObj, paramsSerializer: params => {
    ...
    function renderTracks() {
        ...
        Object.keys(tracks2).forEach((key, index) => {
            promiseArr[index] = new Promise((resolve, reject) => {
                ...
                rodList.push(<div className="box33 radio_style" key={index}>
                    ...
                    <label htmlFor={"8B1" + index}>{pageLanguage1 === 'fa' ? DesignName : DesignEnName}
                        <br/><p className="surcharge_price">{Price > 0 ? ((pageLanguage1 === "en" ? "Add " : "+ ") + GetPrice(Price, pageLanguage1, t("TOMANS"))) : (pageLanguage1 === "en" ? "Surcharge Applies" : "+ شامل هزینه")}</p>
                    </label>
                </div>);
        ...
        Object.keys(tracks3).forEach((key, index) => {
            promiseArr2[index] = new Promise((resolve, reject) => {
                let obj = tracks3[key][0] || {};
                let DesignName = convertToPersian(obj["DesignName"]);
                let DesignEnName = obj["DesignENName"];
                let RailId = obj["RailId"];
                let Price = obj["Price"];
    ...
    function handleShow(fabricObj, SwatchDetailId) {
        ...
        const tempDiv = [];
        const tempDiv1 = [];
        tempDiv.push(<div key={PhotoPath} className="zoomImg">
            <div className="imageContainer">
                <ReactImageMagnify
                    imageProps={{
                        alt: '', // isFluidWidth: true,
                        src: `https://api.atlaspood.ir/${PhotoPath}`
                    }}
                    magnifiedImageProps={{
                        src: `https://api.atlaspood.ir/${PhotoPath}`, width: 800, height: 800
                    }}
        ...
        tempDiv1.push(<div key={1} className="zoom_modal_header_container">
    ...
    function ContextAwareToggle({stepNum, stepTitle, stepTitle2, stepSelected, eventKey, callback, stepRef, type, required, cartCustomText}) {
        ...
        return (<div
            ...
            {required && stepSelected === "" && <div className="stepRequired"/>}
        </div>);
    }
    ...
    function NextStep({children, eventKey, callback, onClick}) {
        ...
        return (<div className="nextStep_area" onClick={onClick}>
    ...
    function optionSelectChanged_three(obj, refIndex, position, isMin, modalRef, postfixEn, postfixFa, pageLang) {
        if (obj !== undefined) {
            ...
            if (temp.labels[refIndex] === undefined) temp.labels[refIndex] = [];
            if (temp.values[refIndex] === undefined) temp.values[refIndex] = [];
        ...
        if (isLoggedIn) {
            ...
            if (Object.keys(tempProjectContainer).length !== 0) {
                ...
                if (minusPlus !== undefined) {
                    if (tempProject["Count"] + minusPlus <= 0 || tempProject["Count"] + minusPlus > 10) setBasketNumber(cart, refIndex, tempProject["Count"] + minusPlus, type); else {
            ...
            if (temp.length > 0) {
                if (minusPlus !== undefined) {
                    if (temp[refIndex] === undefined) {
                        ...
                        if (temp[refIndex]["Count"] + minusPlus <= 0 || temp[refIndex]["Count"] + minusPlus > 10) setBasketNumber(refIndex, temp[refIndex]["Count"] + minusPlus); else {
    ...
    function clearHardwareSteps() {
        ...
        setStep8C3("");
        // setStep8C4("");
        let tempExtended = JSON.parse(JSON.stringify(extendedTitle));
        tempExtended["8"]= [];
        tempExtended["8A"]= [];
        tempExtended["8B"]= [];
        tempExtended["8C"]= [];
    ...
    function setCart(refIndex, cartValue, delRefs, secondRefIndex, secondCartValue, customAccCart) {
        // console.log(refIndex, cartValue, delRefs, secondRefIndex, secondCartValue, customAccCart , pageLoad, rodsLoad, rodsLoad2);
        let temp = {
            ...{
                "Mount": "Ceiling"
        ...
        if (delRefs !== undefined) {
            ...
            tempArr.forEach(ref => {
                if (ref !== undefined) {
                    if (temp[ref] !== undefined) delete temp[ref];
                }
            });
        }
        ...
        // console.log(refIndex, temp[refIndex]);
        // setTimeout(() => {
        // console.log(temp);
        // }, 1000);
        let tempPostObj = {};
        ...
        let promise2 = new Promise((resolve, reject) => {
            if (stepSelectedValue["3"] !== undefined && !pageLoad && !rodsLoad && !rodsLoad2 && refIndex !== "ZipCode" && delRefs !== "PanelType") {
                ...
                axios.post(baseURLPrice, tempPostObj)
                    ...
                    setPrice(0);
                    if (temp["HeightCart"] !== undefined) delete temp["HeightCart"];
                    if (temp["WidthCart"] !== undefined) delete temp["WidthCart"];
                    setWindowSize("");
                    setFabricQty(0);
                    setLiningPrice(0);
                    resolve();
        ...
        promise2.then(() => {
            if (!pageLoad && !rodsLoad && !rodsLoad2 && refIndex !== undefined) {
                setCartValues(temp);
            }
    ...
    function selectUncheck(e) {
        ...
        let tempValue = JSON.parse(JSON.stringify(stepSelectedValue));
        if (tempValue[refIndex] !== undefined) delete tempValue[refIndex];
        setStepSelectedValue(tempValue);
                    ...
                    if (draperies.length) {
                        draperies.sort(function (a, b) {
                            ...
                            if (obj["SewingModelId"] === "0326") {
                                temp1[i] = <li className="custom_cart_item" key={"drapery" + i} ref={ref => (draperyRef.current[draperies[i]["CartDetailId"]] = ref)}>
                                                        ...
                                                        <Accordion.Item eventKey="0">
                                                            ...
                                                            <Accordion.Body className="basket_item_title_dropdown dk_curtain_preview_dropdown">
                                                                <div className="dk_curtain_preview_detail_container">
                                                                    {sodFabrics.map((item, i) => <div key={i}
                ...
                let promise2 = new Promise((resolve, reject) => {
                    if (swatches.length) {
                        for (let i = 0; i < swatches.length; i++) {
                            let obj = swatches[i];
                            temp1[i + draperies.length] = <li className="custom_cart_item" key={"swatches" + i} ref={ref => (draperyRef.current[swatches[i]["CartDetailId"]] = ref)}>
                                <div className="custom_cart_item_image_container">
                                    <img src={`https://api.atlaspood.ir/${obj["PhotoUrl"]}`} alt="" className="custom_cart_item_img img-fluid"/>
                                </div>
                                <div className="custom_cart_item_desc">
                    ...
                    promiseArr[0] = new Promise((resolve, reject) => {
                        ...
                        Promise.all(promiseArr2).then(function (values) {
                            ...
                            cartObjects["drapery"].forEach((obj1, index) => {
                                ...
                                let roomName = (obj["WindowName"] === undefined || obj["WindowName"] === "") ? "" : " / " + obj["WindowName"];
                                if (obj["SewingModelId"] === "0326") {
                                    temp1[cartObjects["drapery"].length - index - 1] = <li className="custom_cart_item" key={"drapery" + index} ref={ref => (draperyRef.current[index] = ref)}>
                                        <div className="custom_cart_item_image_container">
                                            <img src={`https://api.atlaspood.ir/${obj["PhotoUrl"]}`} alt="" className="custom_cart_item_img img-fluid"/>
                                        </div>
                                        <div className="custom_cart_item_desc">
        ...
        GetUserProjectData(data, isLoggedIn, editIndex, changeLang).then((temp) => {
                ...
                if (temp["FabricId"]) {
                    axios.get(baseURLFabrics, {
                        ...
                        response.data.forEach(obj => {
                            ...
                            if (obj["FabricId"] === temp["SheersId1"]) {
                                tempSheers = obj;
                            }
                            if (obj["FabricId"] === temp["SheersId2"]) {
                                tempSheers2 = obj;
                            }
                        });
                        resolve();
            ...
            promise2.then(() => {
                if (Object.keys(tempFabric).length > 0 || Object.keys(tempFabric2).length > 0) {
                    ...
                    if (Object.keys(tempSheers).length > 0) {
                        let temp1 = JSON.parse(JSON.stringify(sheersSelected));
                        temp1.selectedFabricId = tempSheers.FabricId;
                        temp1.selectedTextEn = tempSheers.DesignEnName;
                        temp1.selectedTextFa = tempSheers.DesignName;
                        temp1.selectedColorEn = tempSheers.ColorEnName;
                        temp1.selectedColorFa = tempSheers.ColorName;
                        temp1.selectedHasTrim = tempSheers.HasTrim;
                        temp1.selectedPhoto = tempSheers.FabricOnModelPhotoUrl;
                        setSheersSelected(temp1);
                        // fabricClicked(tempFabric["FabricOnModelPhotoUrl"], tempFabric["HasTrim"]);
                        depSetTempArr = new Set([...setGetDeps("", "2B1", depSetTempArr)]);
                    } else if (Object.keys(tempSheers2).length > 0) {
                        let temp1 = JSON.parse(JSON.stringify(sheersSelected2));
                        temp1.selectedFabricId = tempSheers2.FabricId;
                        temp1.selectedTextEn = tempSheers2.DesignEnName;
                        temp1.selectedTextFa = tempSheers2.DesignName;
                        temp1.selectedColorEn = tempSheers2.ColorEnName;
                        temp1.selectedColorFa = tempSheers2.ColorName;
                        temp1.selectedHasTrim = tempSheers2.HasTrim;
                        temp1.selectedPhoto = tempSheers2.FabricOnModelPhotoUrl;
                        setSheersSelected2(temp1);
                        // fabricClicked(tempFabric["FabricOnModelPhotoUrl"], tempFabric["HasTrim"]);
                        depSetTempArr = new Set([...setGetDeps("", "2B1", depSetTempArr)]);
                    } else {
                        depSetTempArr = new Set([...setGetDeps("2B1", "", depSetTempArr)]);
                ...
                if (temp["PrivacyLayer"]) {
                    ...
                    if (temp["PrivacyLayer"] === "None") {
                        ...
                        depSetTempArr = new Set([...setGetDeps("", "2B", depSetTempArr)]);
                    } else {
                    ...
                    if(temp["PrivacyLayer"] !== "None") {
                        if (temp["PrivacyLayerHeaderStyle"]) {
                            setStep2C(temp["PrivacyLayerHeaderStyle"]);
                            if (temp["PrivacyLayerHeaderStyle"] === "Grommet") {
                                let refIndex = inputs.current["2C1"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["2C1"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["2C1"].value;
                                depSetTempArr = new Set([...setGetDeps("", "2C", depSetTempArr)]);
                                setSheersModelId2("0099");
                            } else if (temp["PrivacyLayerHeaderStyle"] === "Inverted Box Pleat") {
                                let refIndex = inputs.current["2C2"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["2C2"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["2C2"].value;
                                depSetTempArr = new Set([...setGetDeps("", "2C", depSetTempArr)]);
                                setSheersModelId2("0036");
                            } else if (temp["PrivacyLayerHeaderStyle"] === "Pencil Pleat") {
                                let refIndex = inputs.current["2C3"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["2C3"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["2C3"].value;
                                depSetTempArr = new Set([...setGetDeps("", "2C", depSetTempArr)]);
                                setSheersModelId2("0001");
                            } else {
                                let refIndex = inputs.current["2C4"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["2C4"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["2C4"].value;
                                depSetTempArr = new Set([...setGetDeps("", "2C", depSetTempArr)]);
                            }
                        }
                    }
                ...
                if (temp["PanelType"]) {
                    ...
                    if (temp["PanelType"] === "Single Panel, Left") {
                        ...
                        depSetTempArr = new Set([...setGetDeps("", "6", depSetTempArr)]);
                    } else if (temp["PanelType"] === "Pair, Split Draw") {
                ...
                if (temp["Hardware"]) {
                    ...
                    if (temp["Hardware"] === "None") {
                        ...
                        tempExtended["8"]= [inputs.current["82"].getAttribute('text')];
                        
                        let tempMount = changeLang ? temp["Mount8"] : temp["Mount"];
                        if (temp["RailDesign"]) {
                            setStep81(temp["RailDesign"]);
                            setTimeout(() => {
                                if (temp["RailId"]) {
                                    setStep82(temp["RailId"]);
                                }
                                if (temp["BatonOption"]) {
                                    setStep83(temp["BatonOption"]);
                                    if (temp["BatonOption"] === "None") {
                                    ...
                                    } else if (temp["BatonOption"] === "Baton 30cm") {
                                    ...
                                    } else if (temp["BatonOption"] === "Baton 45cm") {
                                        ...
                                        tempExtended["8"][3]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("Cord")}</span></li>;
                                    }
                                }
                                if (tempMount) {
                                    setStep84(tempMount);
                        ...
                        depSetTempArr = new Set([...setGetDeps((temp["RailDesign"] !== undefined ? "" : "81,") + (temp["RailId"] !== undefined ? "" : "82,") + (temp["BatonOption"] !== undefined ? "" : "83,") + (tempMount !== undefined ? "" : "84,"), "8", depSetTempArr)]);
                        
                    } else {
                        let refIndex = inputs.current["83"].getAttribute('ref-num');
                        tempLabels[refIndex] = inputs.current["83"].getAttribute('text');
                        tempValue[refIndex] = inputs.current["83"].value;
                        depSetTempArr = new Set([...setGetDeps("8A,8B"+(temp["PrivacyLayer"] === "None" ? "" : "8C,"), "8", depSetTempArr)]);
                        tempExtended["8"]= [inputs.current["83"].getAttribute('text')];
                        
                        if (temp["DraperyHardware"]) {
                            setStep8A(temp["DraperyHardware"]);
                            if (temp["DraperyHardware"] === "None") {
                                let refIndex = inputs.current["8A1"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8A1"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8A1"].value;
                                tempExtended["8A"]= [];
                                depSetTempArr = new Set([...setGetDeps("", "8A", depSetTempArr)]);
                            } else {
                                let refIndex = inputs.current["8A2"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8A2"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8A2"].value;
                                tempExtended["8A"]= [inputs.current["8A2"].getAttribute('text')];
                                
                                let tempMount = changeLang ? temp["MountA"] : temp["Mount"];
                                if (temp["RailDesignA"]) {
                                    setStep8A1(temp["RailDesignA"]);
                                    setTimeout(() => {
                                        if (temp["RailIdA"]) {
                                            setStep8A2(temp["RailIdA"]);
                                        }
                                        if (temp["BatonOptionA"]) {
                                            setStep8A3(temp["BatonOptionA"]);
                                            if (temp["BatonOptionA"] === "None") {
                                                tempExtended["8A"][2]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("None")}</span></li>;
                                            } else if (temp["BatonOptionA"] === "Baton 30cm") {
                                                tempExtended["8A"][2]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("Baton 30cm")}</span></li>;
                                            } else if (temp["BatonOptionA"] === "Baton 45cm") {
                                                tempExtended["8A"][2]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("Baton 45cm")}</span></li>;
                                            } else {
                                                tempExtended["8A"][2]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("Cord")}</span></li>;
                                            }
                                        }
                                        if (tempMount) {
                                            setStep8A4(tempMount);
                                            if (tempMount === "Wall") {
                                                tempExtended["8A"][3]= <li key="4" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Mount Type")}</span><span className="step_title_extended_list_item_text">{t("Wall")}</span></li>;
                                            } else {
                                                tempExtended["8A"][3]= <li key="4" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Mount Type")}</span><span className="step_title_extended_list_item_text">{t("Ceiling")}</span></li>;
                                            }
                                        }
                                    }, 200);
                                }
                                depSetTempArr = new Set([...setGetDeps((temp["RailDesignA"] !== undefined ? "" : "8A1,") + (temp["RailIdA"] !== undefined ? "" : "8A2,") + (temp["BatonOptionA"] !== undefined ? "" : "8A3,") + (tempMount !== undefined ? "" : "8A4,"), "8A", depSetTempArr)]);
                            }
                        }
                        
                        if (temp["SheerHardware"]) {
                            setStep8B(temp["SheerHardware"]);
                            if (temp["SheerHardware"] === "None") {
                                let refIndex = inputs.current["8B1"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8B1"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8B1"].value;
                                tempExtended["8B"]= [];
                                depSetTempArr = new Set([...setGetDeps("", "8B", depSetTempArr)]);
                            } else if (temp["SheerHardware"] === "Track") {
                                let refIndex = inputs.current["8B2"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8B2"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8B2"].value;
                                tempExtended["8B"]= [inputs.current["8B2"].getAttribute('text')];
                                
                                // let tempMount = changeLang ? temp["MountB"] : temp["Mount"];
                                if (temp["RailDesignB"]) {
                                    setStep8B1(temp["RailDesignB"]);
                                    setTimeout(() => {
                                        if (temp["RailIdB"]) {
                                            setStep8B2(temp["RailIdB"]);
                                        }
                                        if (temp["BatonOptionB"]) {
                                            setStep8B3(temp["BatonOptionB"]);
                                            if (temp["BatonOptionB"] === "None") {
                                            ...
                                            } else if (temp["BatonOptionB"] === "Baton 30cm") {
                                            ...
                                            } else if (temp["BatonOptionB"] === "Baton 45cm") {
                                        ...
                                        // if (tempMount) {
                                        //     setStep8B4(tempMount);
                                        // }
                                        
                                        if (temp["hasPower"]) {
                                            setStep8B5(temp["hasPower"]);
                                ...
                                depSetTempArr = new Set([...setGetDeps((temp["hasPower"] ? ((temp["MotorPosition"] ? "" : "8B51,")) : "8B5,") + (temp["RailDesignB"] !== undefined ? "" : "8B1,") + (temp["RailIdB"] !== undefined ? "" : "8B2,") + (temp["BatonOptionB"] !== undefined ? "" : "8B3,"), "8B", depSetTempArr)]);
                            } else {
                                let refIndex = inputs.current["8B3"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8B3"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8B3"].value;
                                tempExtended["8B"]= [inputs.current["8B3"].getAttribute('text')];
                                ...
                                depSetTempArr = new Set([...setGetDeps((temp["RailDesignB"] !== undefined ? "" : "8B1,") + (temp["RailIdB"] !== undefined ? "" : "8B2,") + (temp["BatonOptionB"] !== undefined ? "" : "8B3,"), "8B", depSetTempArr)]);
                            }
                        }
                        
                        if (temp["PrivacyLayerHardware"]) {
                            setStep8C(temp["PrivacyLayerHardware"]);
                            if (temp["PrivacyLayerHardware"] === "None") {
                                let refIndex = inputs.current["8C1"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8C1"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8C1"].value;
                                tempExtended["8C"]= [];
                                depSetTempArr = new Set([...setGetDeps("", "8C", depSetTempArr)]);
                            } else if (temp["PrivacyLayerHardware"] === "Track") {
                                let refIndex = inputs.current["8C2"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8C2"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8C2"].value;
                                tempExtended["8C"]= [inputs.current["8C2"].getAttribute('text')];
                                
                                // let tempMount = changeLang ? temp["MountC"] : temp["Mount"];
                                if (temp["RailDesignC"]) {
                                    setStep8C1(temp["RailDesignC"]);
                                    setTimeout(() => {
                                        if (temp["RailIdC"]) {
                                            setStep8C2(temp["RailIdC"]);
                                        }
                                        if (temp["BatonOptionC"]) {
                                            setStep8C3(temp["BatonOptionC"]);
                                            if (temp["BatonOptionC"] === "None") {
                                            ...
                                            } else if (temp["BatonOptionC"] === "Baton 30cm") {
                                            ...
                                            } else if (temp["BatonOptionC"] === "Baton 45cm") {
                                        ...
                                        // if (tempMount) {
                                        //     setStep8C4(tempMount);
                                        // }
                                        
                                        if (temp["hasPower"]) {
                                            setStep8C5(temp["hasPower"]);
                                            setSelectedMotorPosition(temp["MotorPosition"] ? [{
                                                value: temp["MotorPosition"],
                                                label: MotorPosition[pageLanguage].find(opt => opt.value === temp["MotorPosition"]).label
                                            }] : []);
                                ...
                                depSetTempArr = new Set([...setGetDeps((temp["hasPower"] ? ((temp["MotorPosition"] ? "" : "8C51,")) : "8C5,") + (temp["RailDesignC"] !== undefined ? "" : "8C1,") + (temp["RailIdC"] !== undefined ? "" : "8C2,") + (temp["BatonOptionC"] !== undefined ? "" : "8C3,"), "8C", depSetTempArr)]);
                            } else {
                                let refIndex = inputs.current["8C3"].getAttribute('ref-num');
                                tempLabels[refIndex] = inputs.current["8C3"].getAttribute('text');
                                tempValue[refIndex] = inputs.current["8C3"].value;
                                tempExtended["8C"]= [inputs.current["8C3"].getAttribute('text')];
                                ...
                                depSetTempArr = new Set([...setGetDeps((temp["RailDesignC"] !== undefined ? "" : "8C1,") + (temp["RailIdC"] !== undefined ? "" : "8C2,") + (temp["BatonOptionC"] !== undefined ? "" : "8C3,"), "8C", depSetTempArr)]);
                            }
                        }
                    }
                    setStepSelectedLabel(tempLabels);
                ...
                if (temp["RoomNameEn"]) {
                    ...
                    depSetTempArr = new Set([...setGetDeps("", "1001", depSetTempArr)]);
                    setSelectedRoomLabel(temp["RoomNameEn"] ? [{
                        value: temp["RoomNameEn"], label: rooms[pageLanguage].find(opt => opt.value === temp["RoomNameEn"]).label
                    }] : []);
                ...
                if (temp["WindowName"] && temp["WindowName"] !== "") {
                    ...
                    depSetTempArr = new Set([...setGetDeps("", "1002", depSetTempArr)]);
    ...
    function filterSelected(filter, filterId, isDelete) {
        ...
        if (filter === 1) {
            if (isDelete) {
                setFilterColors([...filterColors, filterId]);
            } else {
                setFilterColors(filterColors.filter((id1) => id1 !== filterId),);
            }
        } else if (filter === 2) {
            if (isDelete) {
                setFilterPatterns([...filterPatterns, filterId]);
            } else {
                setFilterPatterns(filterPatterns.filter((id1) => id1 !== filterId),);
            }
        } else if (filter === 3) {
            if (isDelete) {
                setFilterTypes([...filterTypes, filterId]);
            } else {
                setFilterTypes(filterTypes.filter((id1) => id1 !== filterId),);
            }
        } else {
            if (isDelete) {
                setFilterPrices([...filterPrices, filterId]);
            } else {
                setFilterPrices(filterPrices.filter((id1) => id1 !== filterId),);
            }
        }
    }
    
    const optionsMetalValance = {
        "en": [{value: 'White', label: 'White'}, {value: 'Silver', label: 'Silver'}, {value: 'Black', label: 'Black'}], "fa": [{value: 'White', label: 'سفید'}, {value: 'Silver', label: 'نقره ای'}, {value: 'Black', label: 'مشکی'}],
    ...
    const [MotorType, setMotorType] = useState({
        "en": [{value: 'Standard', label: 'Standard'}, {value: 'Smart', label: 'Smart'}], "fa": [{value: 'Standard', label: 'استاندارد'}, {value: 'Smart', label: 'هوشمند'}],
    ...
    //         let tempObj = {
    //             "en": [{
    //                 value: 'Standard', label: (modelAccessories["17"] && modelAccessories["17"]["1"] && qty > modelAccessories["17"]["1"]["FromQty"] && qty <= modelAccessories["17"]["1"]["ToQty"]) ? "Standard (" + GetPrice(modelAccessories["17"]["1"]["Price"], pageLanguage, t("TOMANS")) + ")" : ((modelAccessories["17"] && modelAccessories["17"]["14"] && qty > modelAccessories["17"]["14"]["FromQty"] && qty <= modelAccessories["17"]["14"]["ToQty"]) ? "Standard (" + GetPrice(modelAccessories["17"]["14"]["Price"], pageLanguage, t("TOMANS")) + ")" : ""), apiAccValue: (modelAccessories["17"] && modelAccessories["17"]["1"] && qty > modelAccessories["17"]["1"]["FromQty"] && qty <= modelAccessories["17"]["1"]["ToQty"]) ? {
    //                     "SewingAccessoryId": 17, "SewingModelAccessoryId": 0, "SewingAccessoryValue": "61500508", "Qty": 1
    //                 } : ((modelAccessories["17"] && modelAccessories["17"]["14"] && qty > modelAccessories["17"]["14"]["FromQty"] && qty <= modelAccessories["17"]["14"]["ToQty"]) ? {
    //                     "SewingAccessoryId": 17, "SewingModelAccessoryId": 0, "SewingAccessoryValue": "61500507", "Qty": 1
    //                 } : "")
    //             }, {
    //                 value: 'Smart', label: (modelAccessories["18"] && modelAccessories["18"]["15"] && qty > modelAccessories["18"]["15"]["FromQty"] && qty <= modelAccessories["18"]["15"]["ToQty"]) ? "Smart (" + GetPrice(modelAccessories["18"]["15"]["Price"], pageLanguage, t("TOMANS")) + ")" : ((modelAccessories["18"] && modelAccessories["18"]["16"] && qty > modelAccessories["18"]["16"]["FromQty"] && qty <= modelAccessories["18"]["16"]["ToQty"]) ? "Smart (" + GetPrice(modelAccessories["18"]["16"]["Price"], pageLanguage, t("TOMANS")) + ")" : ""), apiAccValue: (modelAccessories["18"] && modelAccessories["18"]["15"] && qty > modelAccessories["18"]["15"]["FromQty"] && qty <= modelAccessories["18"]["15"]["ToQty"]) ? {
    //                     "SewingAccessoryId": 18, "SewingModelAccessoryId": 0, "SewingAccessoryValue": "61500518", "Qty": 1
    //                 } : ((modelAccessories["18"] && modelAccessories["18"]["16"] && qty > modelAccessories["18"]["16"]["FromQty"] && qty <= modelAccessories["18"]["16"]["ToQty"]) ? {
    //                     "SewingAccessoryId": 18, "SewingModelAccessoryId": 0, "SewingAccessoryValue": "61500517", "Qty": 1
    //                 } : "")
    //             }], "fa": [{
    //                 value: 'Standard', label: (modelAccessories["17"] && modelAccessories["17"]["1"] && qty > modelAccessories["17"]["1"]["FromQty"] && qty <= modelAccessories["17"]["1"]["ToQty"]) ? "استاندارد (" + GetPrice(modelAccessories["17"]["1"]["Price"], pageLanguage, t("TOMANS")) + ")" : ((modelAccessories["17"] && modelAccessories["17"]["14"] && qty > modelAccessories["17"]["14"]["FromQty"] && qty <= modelAccessories["17"]["14"]["ToQty"]) ? "استاندارد (" + GetPrice(modelAccessories["17"]["14"]["Price"], pageLanguage, t("TOMANS")) + ")" : ""), apiAccValue: (modelAccessories["17"] && modelAccessories["17"]["1"] && qty > modelAccessories["17"]["1"]["FromQty"] && qty <= modelAccessories["17"]["1"]["ToQty"]) ? {
    //                     "SewingAccessoryId": 17, "SewingModelAccessoryId": 0, "SewingAccessoryValue": "61500508", "Qty": 1
    //                 } : ((modelAccessories["17"] && modelAccessories["17"]["14"] && qty > modelAccessories["17"]["14"]["FromQty"] && qty <= modelAccessories["17"]["14"]["ToQty"]) ? {
    //                     "SewingAccessoryId": 17, "SewingModelAccessoryId": 0, "SewingAccessoryValue": "61500507", "Qty": 1
    //                 } : "")
    ...
    //                 value: selectedMotorType[0].value, label: tempObj[pageLanguage].find(opt => opt.value === selectedMotorType[0].value).label
    //             }] : []);
    //             if (motorLoad) {
    //                 setTimeout(() => {
    //                     setMotorLoad(false);
    ...
    const MotorPosition = {
        "en": [{value: 'Left', label: 'Left'}, {value: 'Right', label: 'Right'}], "fa": [{value: 'Left', label: 'چپ'}, {value: 'Right', label: 'راست'}],
        
    };
    
    const qty = {
        "en": [{value: '0', label: '0'}, {value: '1', label: '1'}, {value: '2', label: '2'}, {value: '3', label: '3'}, {value: '4', label: '4'}, {value: '5', label: '5'}, {value: '6', label: '6'}, {value: '7', label: '7'}, {value: '8', label: '8'}, {value: '9', label: '9'}, {value: '10', label: '10'}], "fa": [{value: '0', label: '۰'}, {value: '1', label: '۱'}, {value: '2', label: '۲'}, {value: '3', label: '۳'}, {value: '4', label: '۴'}, {value: '5', label: '۵'}, {value: '6', label: '۶'}, {value: '7', label: '۷'}, {value: '8', label: '۸'}, {value: '9', label: '۹'}, {value: '10', label: '۱۰'}],
    };
    
    const motorChannels = {
        "en": [{value: '0', label: '0 (All Products)', disabled: true}, {value: '1', label: '1'}, {value: '2', label: '2'}, {value: '3', label: '3'}, {value: '4', label: '4'}, {value: '5', label: '5'}, {value: '6', label: '6'}, {value: '7', label: '7'}, {value: '8', label: '8'}, {value: '9', label: '9'}, {value: '10', label: '10'}, {value: '11', label: '11'}, {value: '12', label: '12'}, {value: '13', label: '13'}, {value: '14', label: '14'}, {value: '15', label: '15'}], "fa": [{value: '0', label: '۰ (همه محصولات)'}, {value: '1', label: '۱'}, {value: '2', label: '۲'}, {value: '3', label: '۳'}, {value: '4', label: '۴'}, {value: '5', label: '۵'}, {value: '6', label: '۶'}, {value: '7', label: '۷'}, {value: '8', label: '۸'}, {value: '9', label: '۹'}, {value: '10', label: '۱۰'}, {value: '11', label: '۱۱'}, {value: '12', label: '۱۲'}, {value: '13', label: '۱۳'}, {value: '14', label: '۱۴'}, {value: '15', label: '۱۵'}],
    ...
    const colors = {
        "en": [{value: 'Aqua', label: 'Aqua'}, {value: 'Beige', label: 'Beige'}, {value: 'Black', label: 'Black'}, {value: 'Blue', label: 'Blue'}, {value: 'Brown', label: 'Brown'}, {value: 'Gold', label: 'Gold'}, {value: 'Green', label: 'Green'}, {value: 'Grey', label: 'Grey'}, {value: 'Orange', label: 'Orange'}, {value: 'Pink', label: 'Pink'}], "fa": [{value: 'Aqua', label: 'آبی آسمانی'}, {value: 'Beige', label: 'بژ'}, {value: 'Black', label: 'مشکلی'}, {value: 'Blue', label: 'آبی'}, {value: 'Brown', label: 'قهوه ای'}, {value: 'Gold', label: 'طلایی'}, {value: 'Green', label: 'سبز'}, {value: 'Grey', label: 'خاکستری'}, {value: 'Orange', label: 'نارنجی'}, {value: 'Pink', label: 'صورتی'}]
    };
    
    const patterns = {
        "en": [{value: 'Botanical/Florals', label: 'Botanical/Florals'}, {value: 'Chinoiserie', label: 'Chinoiserie'}, {value: 'Damask', label: 'Damask'}, {value: 'Geometric', label: 'Geometric'}, {value: 'Modern', label: 'Modern'}, {value: 'Solid', label: 'Solid'}, {value: 'Stripes', label: 'Stripes'}], "fa": [{value: 'Botanical/Florals', label: 'بوتانیک'}, {value: 'Chinoiserie', label: 'چینی سری'}, {value: 'Damask', label: 'دیماس'}, {value: 'Geometric', label: 'هندسی'}, {value: 'Modern', label: 'مدرن'}, {value: 'Solid', label: 'یکرنگ'}, {value: 'Stripes', label: 'راه راه'}]
    };
    
    const types = {
        "en": [{value: 'Chenille', label: 'Chenille'}, {value: 'Embroidery', label: 'Embroidery'}, {value: 'Linen', label: 'Linen'}, {value: 'Print', label: 'Print'}, {value: 'Silk', label: 'Silk'}, {value: 'Velvet', label: 'Velvet'}], "fa": [{value: 'Chenille', label: 'شنیلی'}, {value: 'Embroidery', label: 'گلدوزی'}, {value: 'Linen', label: 'کتانی'}, {value: 'Print', label: 'چاپی'}, {value: 'Silk', label: 'ابریشمی'}, {value: 'Velvet', label: 'مخملی'}]
    };
    
    const prices = {
        "en": [{value: 'standard', label: 'standard'}, {value: 'premium', label: 'premium'}, {value: 'Luxe', label: 'Luxe'}], "fa": [{value: 'standard', label: 'معمولی'}, {value: 'premium', label: 'اعلاء'}, {value: 'Luxe', label: 'تجملی'}]
    };
    ...
    useEffect(() => {
        if (selectedMountOutsideType.length) {
            let tempLabels = JSON.parse(JSON.stringify(stepSelectedLabel));
            tempLabels["3A"] = inputs.current["3A1"].getAttribute('text') + " / " + selectedMountOutsideType[0]["value"];
            setStepSelectedLabel(tempLabels);
        }
    }, [selectedMountOutsideType]);
    
    useEffect(() => {
        if (selectedMountOutsideType2.length) {
            let tempLabels = JSON.parse(JSON.stringify(stepSelectedLabel));
            tempLabels["3A"] = inputs.current["3A2"].getAttribute('text') + " / " + selectedMountOutsideType2[0]["value"];
            setStepSelectedLabel(tempLabels);
        }
    }, [selectedMountOutsideType2]);
    
    useEffect(() => {
        if (selectedMountOutsideType3.length) {
            let tempLabels = JSON.parse(JSON.stringify(stepSelectedLabel));
            tempLabels["3A"] = inputs.current["3A3"].getAttribute('text') + " / " + selectedMountOutsideType3[0]["value"];
            setStepSelectedLabel(tempLabels);
        }
    }, [selectedMountOutsideType3]);
    
    useEffect(() => {
        if (selectedMountOutsideType4.length) {
            let tempLabels = JSON.parse(JSON.stringify(stepSelectedLabel));
            tempLabels["3A"] = inputs.current["3A4"].getAttribute('text') + " / " + selectedMountOutsideType4[0]["value"];
            setStepSelectedLabel(tempLabels);
        }
    }, [selectedMountOutsideType4]);
    ...
    useEffect(() => {
        if (pageLanguage !== '') {
            if (Object.keys(rods).length || Object.keys(rods2).length || Object.keys(rods3).length || Object.keys(tracks).length || Object.keys(tracks2).length || Object.keys(tracks3).length) {
                renderRods();
                renderTracks();
            } else {
                setRodsList([]);
                    ...
                    promiseArr[index] = new Promise((resolve, reject) => {
                        ...
                        rodList.push(
                            <div className="box33" key={index}>
                                <div className="radio_group">
                                    <label className="radio_container">
                                        <input className="radio" type="radio" text={pageLanguage === 'fa' ? ColorName : ColorEnName} value={index} name="step8A2" ref-num={"8A2"} id={"8A2" + index} outline="true"
                                               checked={step8A2 === `${RailId}`}
                                               onChange={e => {
                                                   setStep8A2(`${RailId}`);
                                               }} ref={ref => (inputs.current["8A2" + index] = ref)}/>
                                        <div className="frame_img">
                                            <img src={`https://api.atlaspood.ir/${PhotoPath}`} className="img-fluid bracket_color_img" alt=""/>
                                        </div>
                                    </label>
    ...
    useEffect(() => {
        if (step8A1 !== "") {
            ...
            let railObject = {};
            railObject = rails.filter(obj => {
                return obj["RailId"] === step8A1
            })[0] || railObject;
            
            if (railObject["DesignENName"]) {
                // let tempLabels = JSON.parse(JSON.stringify(stepSelectedLabel));
                // tempLabels["8A"] = pageLanguage === "fa" ? railObject["DesignName"] : railObject["DesignENName"];
                // setStepSelectedLabel(tempLabels);
                // tempExtended["8A"][0]= <li key="1" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Design")}</span><span className="step_title_extended_list_item_text">{pageLanguage === "fa" ? convertToPersian(railObject["DesignName"]) : railObject["DesignENName"]}</span></li>;
                let tempExtended = extendedTitle;
                tempExtended["8A"][0] = pageLanguage === "fa" ? convertToPersian(railObject["DesignName"]) : railObject["DesignENName"];
                tempExtended["8A"].splice(1, 3);
                setExtendedTitle(tempExtended);
            }
            setCart("RailDesignA", step8A1, "RailIdA,RailColorFaA,RailColorEnA", "RailDesignFaA,RailDesignEnA", [railObject["DesignName"], railObject["DesignENName"]]);
            showRodsColor(railObject, "8A2");
        }
    }, [step8A1]);
    ...
    useEffect(() => {
        if (step8A2 !== "") {
            ...
            let railObject2 = {};
            railObject2 = rails.filter(obj => {
                return obj["RailId"] === step8A2
            })[0] || railObject2;
            
            if (railObject["DesignENName"] && railObject2["ColorName"] && railObject2["ColorEnName"]) {
                // let tempLabels = JSON.parse(JSON.stringify(stepSelectedLabel));
                // tempLabels["8A"] = pageLanguage === "fa" ? convertToPersian(railObject["DesignName"]) + " / " + convertToPersian(railObject2["ColorName"]) : railObject["DesignENName"] + " / " + railObject2["ColorEnName"];
                // setStepSelectedLabel(tempLabels);
                let tempExtended = extendedTitle;
                tempExtended["8A"][1]= <li key="2" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Color")}</span><span className="step_title_extended_list_item_text">{pageLanguage === "fa" ? convertToPersian(railObject2["ColorName"]) : railObject2["ColorEnName"]}</span></li>;
                setExtendedTitle(tempExtended);
            }
            setCart("RailIdA", step8A2, "", "RailColorFaA,RailColorEnA", [railObject2["ColorName"], railObject2["ColorEnName"]]);
            showRodsColor(railObject, "8A2");
        }
    }, [step8A2]);
    ...
    useEffect(() => {
        setLang().then(() => {
            if (pageLanguage !== '') {
                if (Object.keys(fabrics).length || Object.keys(fabrics2).length) {
                    getCart().then((temp) => {
                        getHasZipcode();
                        setTimeout(() => {
                            renderFabrics(temp);
    ...
    useEffect(() => {
        setLang().then(() => {
            if (pageLanguage !== '') {
                if (Object.keys(rods).length || Object.keys(rods2).length || Object.keys(rods3).length) {
                    renderRods();
                } else {
                    setRodsList([]);
                    setRodsList2([]);
    ...
    useEffect(() => {
        setLang().then(() => {
            if (pageLanguage !== '') {
                if (Object.keys(tracks).length || Object.keys(tracks2).length || Object.keys(tracks3).length) {
                    renderTracks();
                } else {
                    setTracksList([]);
                    setTracksList2([]);
    ...
    useEffect(() => {
        if (firstRender.current) {
            ...
            dispatch({
                type: CartUpdatedTrue, payload: {mainCart: bag}
            });
        }
    }, [bag]);
    ...
    useEffect(() => {
        ...
        GetSewingFilters(1, `${modelID}`).then((temp) => {
            setSewingColors(temp[location.pathname.split('').slice(1, 3).join('')].map((obj, index) => (<Dropdown.Item as={Button} key={index}>
                <label className="dropdown_label">
                    <input type="checkbox" filter-id={obj.value}
                           ...
                           onChange={(e) => {
                               setFilterChanged({
                                   filter: 1, filter_id: e.target.getAttribute("filter-id"), isDelete: e.target.checked
                               });
        ...
        GetSewingFilters(2, `${modelID}`).then((temp) => {
            setSewingPatterns(temp[location.pathname.split('').slice(1, 3).join('')].map((obj, index) => (<Dropdown.Item as={Button} key={index}>
                <label className="dropdown_label">
                    <input type="checkbox" filter-id={obj.value}
                           ref={ref => (filterCheckboxes.current["patterns"] = [...filterCheckboxes.current["patterns"], ref])}
                           onChange={(e) => {
                               setFilterChanged({
                                   filter: 2, filter_id: e.target.getAttribute("filter-id"), isDelete: e.target.checked
                               });
        ...
        GetSewingFilters(3, `${modelID}`).then((temp) => {
            setSewingTypes(temp[location.pathname.split('').slice(1, 3).join('')].map((obj, index) => (<Dropdown.Item as={Button} key={index}>
                <label className="dropdown_label">
                    <input type="checkbox" filter-id={obj.value}
                           ref={ref => (filterCheckboxes.current["types"] = [...filterCheckboxes.current["types"], ref])}
                           onChange={(e) => {
                               setFilterChanged({
                                   filter: 3, filter_id: e.target.getAttribute("filter-id"), isDelete: e.target.checked
                               });
        ...
        GetSewingFilters(4, `${modelID}`).then((temp) => {
            setSewingPrices(temp[location.pathname.split('').slice(1, 3).join('')].map((obj, index) => (<Dropdown.Item as={Button} key={index}>
                <label className="dropdown_label">
                    <input type="checkbox" filter-id={obj.value}
                           ref={ref => (filterCheckboxes.current["prices"] = [...filterCheckboxes.current["prices"], ref])}
                           onChange={(e) => {
                               setFilterChanged({
                                   filter: 4, filter_id: e.target.getAttribute("filter-id"), isDelete: e.target.checked
                               });
    ...
    return (<div className={`Custom_model_container ${pageLanguage === 'fa' ? "font_farsi" : "font_en"}`}>
        ...
        <div className="models_title_div">
            ...
            {defaultModelDesc && defaultModelDesc !== "" && <h2>{pageLanguage === 'fa' ? convertToPersian(defaultModelDescFa) : defaultModelDesc}</h2>}
        </div>
        <div className="model_customize_container">
            <div className={offset ? "model_customize_image model_customize_image_fixed" : "model_customize_image"} ref={fixedDiv}>
                <div>
                    {defaultFabricPhoto && <img src={`https://api.atlaspood.ir/${defaultFabricPhoto}`} className="img-fluid" alt=""/>}
                </div>
            </div>
            <div className={`model_customize_section ${pageLanguage === 'fa' ? "font_farsi" : "font_en"}`}>
                <Accordion ref={accordion} flush activeKey={accordionActiveKey}>
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="2" stepNum={t("2")} stepTitle={t("grommet_step2")} stepRef="2" type="1" required={requiredStep["2"]}
                        ...
                        <Accordion.Collapse eventKey="2">
                            <Card.Body>
                                <div className="card_body card-body-fabric">
                                    ...
                                    <NextStep eventKey={step2 !== "" ? "2A" : "2B"}>{t("NEXT STEP")}</NextStep>
                                </div>
                            </Card.Body>
                        </Accordion.Collapse>
                    </Card>
                    ...
                    <Card className={step2 !== "" ? "" : "noDisplay"}>
                        <Card.Header className={step2A !== "" && (!accordionActiveKey.startsWith("2") || accordionActiveKey === "2B" || accordionActiveKey === "2C") ? "hidden" : ""}>
                            <ContextAwareToggle eventKey="2A" stepNum={t("2A")} stepTitle={t("grommet_step2A")} stepRef="2A" type="1" required={requiredStep["2A"]}
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="2B" stepNum={t("3")} stepTitle={t("grommet3_step2")} stepRef="2B" type="1" required={requiredStep["2B"]}
                        ...
                        <Accordion.Collapse eventKey="2B">
                            <Card.Body>
                                <div className="card_body card_body_radio card-body-fabric_grommet">
                                    ...
                                    <NextStep eventKey={step2B !== "" && step2B !== "None" ? "2C" : "3"}>{t("NEXT STEP")}</NextStep>
                                </div>
                            </Card.Body>
                        </Accordion.Collapse>
                    </Card>
                    ...
                    <Card className={step2B !== "" && step2B !== "None" ? "" : "noDisplay"}>
                        <Card.Header className={step2C !== "" && (!accordionActiveKey.startsWith("2") || accordionActiveKey === "2A" || accordionActiveKey === "2") ? "hidden" : ""}>
                            <ContextAwareToggle eventKey="2C" stepNum={t("3A")} stepTitle={t("grommet_step2A_sheer")} stepRef="2C" type="1" required={requiredStep["2C"]}
                                ...
                                <div className="card_body card_body_radio card_body_radio_camera card_body_radio_small_img">
                                    <div className="box33 radio_style">
                                        ...
                                        <input className="radio" type="radio" text={t("Grommet")} value="1" name="step2C" ref-num="2C" id="2C1"
                                               ...
                                               onChange={e => {
                                                   if ((step2C === "Inverted Box Pleat" || step2C === "Pencil Pleat") && step8 === "Customize Hardware For Per Curtains") {
                                    ...
                                    <div className="box33 radio_style">
                                        ...
                                        <input className="radio" type="radio" text={t("Inverted Box Pleat")} value="2" name="step2C" ref-num="2C" id="2C2"
                                               ...
                                               onChange={e => {
                                                   if ((step2C === "Grommet" || step2C === "Rod Pocket") && step8 === "Customize Hardware For Per Curtains") {
                                                       modalHandleShow("SheerHeaderStyleWarning2");
                                                       setSheerHeaderStyleTemp2({
                                                           stepValue: "Inverted Box Pleat",
                                                           id: "0036",
                    ...
                    <Card
                        ...
                        <Card.Header className={[...depSet].findIndex(el => el.startsWith("3")) === -1 && (!accordionActiveKey.startsWith("3")) ? "hidden" : ""}>
                            <ContextAwareToggle eventKey="3A" stepNum={t("4A")} stepTitle={t("dk_step2A")} stepRef="3ARod" type="1" required={requiredStep["3ARod"]}
                    ...
                    <Card
                        ...
                        <Card.Header className={[...depSet].findIndex(el => el.startsWith("3")) === -1 && (!accordionActiveKey.startsWith("3")) ? "hidden" : ""}>
                            <ContextAwareToggle eventKey="3B" stepNum={t("4B")} stepTitle={t("grommet_step3BRod")} stepRef="3BRod" type="2" required={requiredStep["3BRod"]}
                    ...
                    <Card
                        ...
                        <Card.Header className={[...depSet].findIndex(el => el.startsWith("3")) === -1 && (!accordionActiveKey.startsWith("3")) ? "hidden" : ""}>
                            <ContextAwareToggle eventKey="3C" stepNum={t("4C")} stepTitle={t("grommet_step3DRod")} stepRef="3CRod" type="2" required={requiredStep["3CRod"]}
                                                stepSelected={stepSelectedLabel["3CRod"] === undefined ? "" : stepSelectedLabel["3CRod"]}/>
                        </Card.Header>
                        <Accordion.Collapse eventKey="3C">
                            <Card.Body>
                    ...
                    <Card
                        ...
                        <Card.Header className={[...depSet].findIndex(el => el.startsWith("3")) === -1 && (!accordionActiveKey.startsWith("3")) ? "hidden" : ""}>
                            <ContextAwareToggle eventKey="3C" stepNum={t("4C")} stepTitle={t("grommet_step3DRodFloor")} stepRef="3CRodFloor" type="2"
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="4" stepNum={t("5")} stepTitle={t("grommet_step4")} stepRef="4" type="1" required={requiredStep["4"]}
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="5" stepNum={t("6")} stepTitle={t("grommet_step5")} stepRef="5" type="1" required={requiredStep["5"]}
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="6" stepNum={t("7")} stepTitle={t("grommet_step6")} stepRef="6" type="1" required={requiredStep["6"]}
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="7" stepNum={t("8")} stepTitle={t("grommet_step7")} stepRef="7" type="1" required={requiredStep["7"]}
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="8" stepNum={t("9")} stepTitle={t("grommet_step8")} stepRef="8" type="1" required={requiredStep["8"]}
                                    ...
                                    <div className="box33 radio_style">
                                        <input className="radio" type="radio" text={t("hardware1")} value="1" name="step8" ref-num="8" id="81" outline="true"
                                               ...
                                               onChange={e => {
                                                   ...
                                                   setStep8("None");
                                                   setHardwareNextStep("9");
                                                   setDeps("", "8,81,82,83,84,8A,8A1,8A2,8A3,8A4,8B,8B1,8B2,8B3,8B5,8B51,8C,8C1,8C2,8C3,8C5,8C51");
                                                   setCart("Hardware", "None", "RailId,BatonOption,RodColor,Mount8,BatonOptionA,RodColorA,MountA,RailIdB,BatonOptionB,RodColorB,MountB,RailIdC,BatonOptionC,RodColorC,MountC,hasPower,MotorPosition,RemoteName,MotorChannels");
                                        ...
                                        <input className="radio" type="radio" text={t("hardware2")} value="2" name="step8" ref-num="8" id="82" outline="true"
                                               ...
                                               onChange={e => {
                                                   ...
                                                   if (step3 === "" || (step3 === "true" && step31 === "")) {
                                                       ...
                                                       setDeps("8", "81,82,83,84,8A,8A1,8A2,8A3,8A4,8B,8B1,8B2,8B3,8C,8C1,8C2,8C3");
                                                       setCart("", "", "Hardware,BatonOptionA,RodColorA,MountA,RailIdB,BatonOptionB,RodColorB,MountB,RailIdC,BatonOptionC,RodColorC,MountC,hasPower,MotorPosition,RemoteName,MotorChannels");
                                                   } else {
                                                       selectChanged(e);
                                                       ...
                                                       setCart("Hardware", "Same Hardware For All Curtains", "BatonOptionA,RodColorA,MountA,RailIdB,BatonOptionB,RodColorB,MountB,RailIdC,BatonOptionC,RodColorC,MountC,hasPower,MotorPosition,RemoteName,MotorChannels");
                                                   }
                                                   setExtendedTitle(tempExtended);
                                        ...
                                        <input className="radio" type="radio" text={t("hardware3")} value="3" name="step8" ref-num="8" id="83" outline="true"
                                               ...
                                               onChange={e => {
                                                   ...
                                                   if (step2A === "" || step2B === "" || (step2B !== "None" && step2B1 === "") || (step2B !== "None" && step2C === "")) {
                                                       ...
                                                       setDeps("8", "81,82,83,84,8A,8A1,8A2,8A3,8A4,8B,8B1,8B2,8B3,8C,8C1,8C2,8C3");
                                                       setCart("", "", "Hardware,RailId,BatonOption,RodColor,Mount8,BatonOptionA,RodColorA,MountA,RailIdB,BatonOptionB,RodColorB,MountB,RailIdC,BatonOptionC,RodColorC,MountC,hasPower,MotorPosition,RemoteName,MotorChannels");
                                                   } else {
                                                       ...
                                                       if(step2B === "None"){
                                                           setDeps("8A,8B", "8,81,82,83,84,8C");
                                                       }
                                                       else{
                                                           setDeps("8A,8B,8C", "8,81,82,83,84");
                                                       }
                                                       setCart("Hardware", "Customize Hardware For Per Curtains", "RailId,BatonOption,RodColor,Mount8");
                                                   }
                                                   setExtendedTitle(tempExtended);
                    ...
                    <Card className={step8 === "Customize Hardware For Per Curtains" ? "" : "noDisplay"}>
                        {/*<Card.Header className={[...depSet].findIndex(el => el.startsWith("8A")) === -1 && (!accordionActiveKey.startsWith("8")) ? "hidden" : ""}>*/}
                        <Card.Header>
                            <ContextAwareToggle eventKey="8A" stepNum={t("9A")} stepTitle={t("grommet_step8A")} stepRef="8A" type="1" required={requiredStep["8A"]}
                        ...
                        <Accordion.Collapse eventKey="8A">
                            <Card.Body>
                                <div className="card_body card_body_radio card_body_hardware">
                                    <div className="box50 radio_style">
                                        <input className="radio" type="radio" text={t("None")} value="1" name="step8A" ref-num="8A" id="8A1"
                                               ...
                                               onChange={e => {
                                                   ...
                                                   setStep8A("None");
                                                   let tempExtended = extendedTitle;
                                                   tempExtended["8A"]=[t("None")];
                                                   setExtendedTitle(tempExtended);
                                                   setDeps("", "8A,8A1,8A2,8A3,8A4");
                                                   setCart("DraperyHardware", "None", "BatonOptionA,RodColorA,MountA");
                                                   selectChanged(e);
                                        ...
                                        <input className="radio" type="radio" text={t("Rod")} value="2" name="step8A" ref-num="8A" id="8A2"
                                               ...
                                               onChange={e => {
                                                   ...
                                                   if (step3 === "" || (step3 === "true" && step31 === "")) {
                                                       ...
                                                       modalHandleShow("noMeasurements");
                                                       setDeps("8A", "8A1,8A2,8A3,8A4");
                                                       setCart("", "", "DraperyHardware,BatonOptionA,RodColorA,MountA");
                                                   } else {
                                                       setStep8A("Rod");
                                                       tempExtended["8A"]=[t("Rod")];
                                                       if (step31 === "false") {
                                                           setDeps("8A1,8A2,8A3", "8A,8A4");
                                                       } else {
                                    ...
                                    {/*</div>*/}
                                    <div
                                        className={step8A !== "None" && step8A !== "" && step8A1 !== "" ? "card_body card_body_radio card_body_baton" : "noDisplay"}>
                                        <div className="box100">
                                            <p className="step_selection_title">{t("Baton Option")}</p>
                                        </div>
                                        <div className="box33 radio_style">
                                            <input className="radio" type="radio" text={t("None")} value="1" name="step8A3" ref-num="8A3" id="8A31"
                                                   checked={step8A3 === "None"}
                                                   onChange={e => {
                                                       setStep8A3("None");
                                                       setDeps("", "8A3");
                                                       setCart("BatonOptionA", "None");
                                                       selectChanged(e);
                                                       let tempExtended = extendedTitle;
                                                       tempExtended["8A"][2]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("None")}</span></li>;
                                                       setExtendedTitle(tempExtended);
                                                   }} ref={ref => (inputs.current["8A31"] = ref)}/>
                                            <label htmlFor="8A31">{t("None")}</label>
                                        </div>
                                        <div className="box33 radio_style">
                                            <input className="radio" type="radio" text={t("Baton 30cm")} value="2" name="step8A3" ref-num="8A3" id="8A32"
                                                   checked={step8A3 === "Baton 30cm"}
                                                   onChange={e => {
                                                       setStep8A3("Baton 30cm");
                                                       setDeps("", "8A3");
                                                       setCart("BatonOptionA", "Baton 30cm");
                                                       selectChanged(e);
                                                       let tempExtended = extendedTitle;
                                                       tempExtended["8A"][2]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("Baton 30cm")}</span></li>;
                                                       setExtendedTitle(tempExtended);
                                                   }} ref={ref => (inputs.current["8A32"] = ref)}/>
                                            <label htmlFor="8A32">{t("Baton 30cm")}<br/><p
                                                className="surcharge_price">{Object.keys(modelAccessories).length !== 0 ? t("Add ") : t("Surcharge Applies")}{(modelAccessories["27"] ? (modelAccessories["27"]["58"] ? GetPrice(modelAccessories["27"]["58"]["Price"], pageLanguage, t("TOMANS")) : null) : null)}</p>
                                            </label>
                                        </div>
                                        <div className="box33 radio_style">
                                            <input className="radio" type="radio" text={t("Baton 45cm")} value="3" name="step8A3" ref-num="8A3" id="8A33"
                                                   checked={step8A3 === "Baton 45cm"}
                                                   onChange={e => {
                                                       setStep8A3("Baton 45cm");
                                                       setDeps("", "8A3");
                                                       setCart("BatonOptionA", "Baton 45cm");
                                                       selectChanged(e);
                                                       let tempExtended = extendedTitle;
                                                       tempExtended["8A"][2]= <li key="3" className="step_title_extended_list_item"><span className="step_title_extended_list_item_title">{t("Baton Option")}</span><span className="step_title_extended_list_item_text">{t("Baton 45cm")}</span></li>;
                                                       setExtendedTitle(tempExtended);
                                                   }} ref={ref => (inputs.current["8A33"] = ref)}/>
                                            <label htmlFor="8A33">{t("Baton 45cm")}<br/><p
                                                className="surcharge_price">{Object.keys(modelAccessories).length !== 0 ? t("Add ") : t("Surcharge Applies")}{(modelAccessories["27"] ? (modelAccessories["27"]["59"] ? GetPrice(modelAccessories["27"]["59"]["Price"], pageLanguage, t("TOMANS")) : null) : null)}</p>
                                            </label>
                                        </div>
                                    </div>
                                    <div
                                        className={step8A !== "None" && step8A !== "" && step8A1 !== "" && step31 !== "false" ? "card_body card_body_radio card_body_rod_mount" : "noDisplay"}>
                                        <div className="box100">
                                            <p className="step_selection_title">{t("step8_rod_mount_title")}</p>
                                        </div>
                                        <div className="box33 radio_style">
                    ...
                    <Card className={step8 === "Customize Hardware For Per Curtains" ? "" : "noDisplay"}>
                        ...
                        <Card.Header>
                            <ContextAwareToggle eventKey="8B" stepNum={t("9B")} stepTitle={t("grommet_step8B")} stepRef="8B" type="1" required={requiredStep["8B"]}
                    ...
                    <Card className={step8 === "Customize Hardware For Per Curtains" && step2B !== "None" ? "" : "noDisplay"}>
                        ...
                        <Card.Header>
                            <ContextAwareToggle eventKey="8C" stepNum={t("9C")} stepTitle={t("grommet_step8C")} stepRef="8C" type="1" required={requiredStep["8C"]}
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="9" stepNum={t("10")} stepTitle={t("grommet_step9")} stepTitle2={t("(Optional)")} stepRef="9" type="1" required={requiredStep["9"]}
                                ...
                                <div className="card_body card_body_Accessories">
                                    <div className="Accessories_container">
                                        <ul className="Accessories_List">
                                            ...
                                            {/*    </div>*/}
                                            {/*    <div className="Accessories_List_item_desc">*/}
                                            {/*        <h1 className="Accessories_List_item_title">Matching Tieback</h1>*/}
                                            ...
                                            {/*            })}><img src={require('../Images/public/minus.svg').default} alt="" className="qty_math_icon"/></button>*/}
                                            {/*            <input type="text" className="qty_num"*/}
                                            {/*                   value={getAccValue(customAcc, "SewingAccessoryValue", "9999999999", pageLanguage)}*/}
                                            {/*                   onChange={(e) => setAccessoriesDesign({*/}
                                            {/*                       "isPlus": undefined,*/}
                                            {/*                       "SewingAccessoryValue": undefined,*/}
                                            {/*                       "SewingAccessoryId": undefined,*/}
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="10" stepNum={t("11")} stepTitle={t("zebra_step6")} stepRef="10" type="2" required={requiredStep["10"]}
                                        ...
                                        <div className="big_box">
                                            <div className="big_box_container box100">
                                                <div className="upload_button_displayer" onClick={() => {
                                                    if (detailsShow) setDetailsShow(false); else setDetailsShow(true);
                                                }}>
                                    ...
                                    <div className="box100 selectInput_section">
                                        ...
                                        <div className="room_select">
                                            ...
                                            <DebounceInput debounceTimeout={500} onKeyDown={() => setCartLoading(true)} type="text" placeholder={t("Window Description")}
                                                           ...
                                                           value={roomLabelText}
                                                           onChange={(e) => {
                                                               if (e.target.value === "") setDeps("1002", ""); else setDeps("", "1002");
                    ...
                    <Card>
                        <Card.Header>
                            <ContextAwareToggle eventKey="11" stepNum={t("12")} stepTitle={t("zebra_step7")} stepTitle2={t("(Optional)")} stepRef="11" type="2"
                                ...
                                <div className="card_body card_body_zipcode">
                                    <div className="zipcode_bag_section">
                                        <div className="zipcode_section_container">
                                            ...
                                            {hasZipcode === "" && <h3 className="zipcode_text">{t("zipcode_text1.5")}</h3>}
                                            ...
                                            {/*    </Accordion>*/}
                                            {/*</div>*/}
                                            {hasZipcode === "" && <div className="zipcode_dropdown_div_container">
                                                <div className="zipcode_input_container">
                                                    <input className="zipcode_input form-control" type="text" name="zipcode_input" value={zipcode}
                                                           placeholder={t("Enter Zip Code")} onChange={(e) => {
                                                        setZipcode(e.target.value.replace(/\D+/g, ''));
                                            ...
                                            {zipcodeButton && <h3 className="zipcode_text2">
                                                {zipcodeButton && hasInstall === false && t("zipcode_text2")}
                                                {zipcodeButton && hasInstall && hasZipcode === "" && t("zipcode_text3")}
                                                {zipcodeButton && hasInstall && hasZipcode !== "" && installPrice === -1 && t("zipcode_text3")}
                                            </h3>}
                                            {zipcodeButton && hasInstall && installPrice === -1 && <h3 className="zipcode_text">{t("zipcode_text8")}</h3>}
                                            {zipcodeButton && hasInstall && !!installPrice && installPrice !== -1 && hasZipcode === "" && <div className="zipcode_available_container">
                                            ...
                                            {zipcodeButton && hasInstall && !!installPrice && installPrice !== -1 && hasZipcode !== "" && <div className="checkbox_style checkbox_style_hasZipcode">
                                                <input type="checkbox" value="1" name="zipcode" disabled={addingLoading} checked={zipcodeChecked === "true"}
                                                       onChange={(e) => {
                                                           if (e.target.checked) {
                                                               setZipcodeChecked("true");
            ...
            <Modal.Body>
                ...
                <div className="buttons_section">
                    ...
                    <button className="btn white_btn" onClick={() => {
                        ...
                        setDeps("", "2A,8,81,82,83,84,8A,8A1,8A2,8A3,8A4,8B,8B1,8B2,8B3,8B5,8B51,8C,8C1,8C2,8C3,8C5,8C51");
                        setCart("SheerHeaderStyle", sheerHeaderStyleTemp.cartValue, "RailId,BatonOption,RodColor,Mount,BatonOptionA,RodColorA,MountA,RailIdB,BatonOptionB,RodColorB,MountB,RailIdC,BatonOptionC,RodColorC,MountC,hasPower,MotorPosition,RemoteName,MotorChannels");
        ...
        <Modal dialogClassName={`warning_modal2 bigSizeModal ${pageLanguage === 'fa' ? "font_farsi" : "font_en"}`}
            ...
            <Modal.Body>
                ...
                <div className="buttons_section">
                    ...
                    <button className="btn white_btn" onClick={() => {
                        setStep2C(sheerHeaderStyleTemp.stepValue);
                        setDeps("", "2C,8,81,82,83,84,8A,8A1,8A2,8A3,8A4,8B,8B1,8B2,8B3,8B5,8B51,8C,8C1,8C2,8C3,8C5,8C51");
                        setCart("PrivacyLayerHeaderStyle", sheerHeaderStyleTemp.cartValue, "RailId,BatonOption,RodColor,Mount,BatonOptionA,RodColorA,MountA,RailIdB,BatonOptionB,RodColorB,MountB,RailIdC,BatonOptionC,RodColorC,MountC,hasPower,MotorPosition,RemoteName,MotorChannels");
                        selectChanged(sheerHeaderStyleTemp.event, "8");
                        setSheersModelId2(sheerHeaderStyleTemp.id);
                        setStep8("");
                        modalHandleClose("SheerHeaderStyleWarning2");
                    }}>{t("I AGREE, CONTINUE ANYWAY")}
                    </button>
        ...
        <Modal dialogClassName={`noMount_modal mediumSizeModal ${pageLanguage === 'fa' ? "font_farsi" : "font_en"}`}
            ...
            <Modal.Body>
                <p>{t("modal_select_noMeasurements")}</p>
        ...
        <Modal backdrop="static" keyboard={false} className={`cart_modal_container cart_agree_container ${pageLanguage === 'fa' ? "font_farsi" : "font_en"}`}
            ...
            <Modal.Header>
                {cartStateAgree && <span className="custom_cart_title">{t("My Bag")} <h3>({pageLanguage === "fa" ? NumberToPersianWord.convertEnToPe(`${cartCount}`) : cartCount})</h3></span>}
                {!cartStateAgree && <p className="custom_cart_title">&nbsp;</p>}
            ...
            <Modal.Body>
                {cartStateAgree && <ul className="custom_cart_items_container">
                    {cartItems}
                </ul>}
                
                {!cartStateAgree && <h1 className="cart_agree_title1">{t("REVIEW ORDER")}</h1>}
                {!cartStateAgree && <h2 className="cart_agree_title2">{t("TERMS OF SALE")}</h2>}
                {!cartStateAgree && <div>{cartAgree}</div>}
                {!cartStateAgree && <span className="cart_agree_desc">
                        <div className="checkbox_style">
                ...
                {!cartStateAgree && <div className="go_to_checkout">
        ...
        <Modal backdrop="static" keyboard={false}
            ...
            <Modal.Body>
                {projectModalState === 0 && <div className="add_to_project_container2">
                    ...
                    <div className="box100 selectInput_section">
                        ...
                        <div className="room_select">
                            ...
                            <DebounceInput debounceTimeout={500} onKeyDown={() => setCartLoading(true)} type="text" placeholder={t("Window Description")}
                                           ...
                                           value={roomLabelText}
                                           onChange={(e) => {
                                               if (e.target.value === "") setDeps("62", ""); else setDeps("", "62");
                    ...
                    {!!(roomLabelText !== "" && selectedRoomLabel.length) && <button className="save_item_btn btn-new-dark"
                                                                                     onClick={() => {
                                                                                         if (roomLabelText !== "" && selectedRoomLabel.length) {
                ...
                {projectModalState === 1 && <div className="add_to_project_container">
                ...
                {projectModalState === 2 && <ModalLogin/>}
            </Modal.Body>
        </Modal>
